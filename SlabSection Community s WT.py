...
with open('Data/Down Dip/Community Data.json') as json_file:
...
title = 'Down Slab - All Communities'
...
for community in comm_dict.keys():
    ...
    for i in range(len(s_ns)):
        if midpoints[i] < xmax:
            ax.text(midpoints[i],  comm_n[i]+0.22, s_ns[i], c='gray', fontsize=6, ha='center', va='bottom', alpha=0.75)
            # if s_ns[i] < 7:
            #     ax.scatter(midpoints[i], comm_n[i], c='purple', s=10)
...
# plt.tight_layout()
plt.savefig(f'Figures/Down Dip/{figure_fname}', bbox_inches='tight')
