...
class Earthquake:
    ...
    vel_s = vel_p * 0.6
    vel_surf = vel_s * 0.9
    # time to process and send out an earthquake warning (s) (calculation, processing, and data latency)
    ttp = 4
    ...
    def __init__(self, xml='grid.xml'):
        ...
        self.distances_hypo = np.array(
            [[np.sqrt(np.square(self.distances_epi) + np.square(self.event_depth))]]
        )
        self.arrivals_p = self.distances_hypo / Earthquake.vel_p
        self.arrivals_s = self.distances_epi / Earthquake.vel_s
